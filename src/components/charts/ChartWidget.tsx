        ...
        series: [
          {
            type: seriesType,
            data: vals,
            smooth: widget.type !== 'bar',
            areaStyle: widget.type === 'area' ? { opacity: 0.15 } : undefined,
            itemStyle: { color, borderRadius: widget.type === 'bar' ? [4, 4, 0, 0] : 0 },
            lineStyle: { width: 2.5 },
          },
        ],
        graphic:
  ...
  if (widget.type === 'kpi') {
    const live =
      data?.length && widget.dataSource?.valueField
        ...
        : undefined;
    const display = live != null && live !== '' ? live : widget.staticValue ?? '—';
    return (
      <div className={cn('h-full flex flex-col justify-center px-1', className)}>
        <p className="text-3xl sm:text-4xl font-bold tracking-tight text-white">{String(display)}</p>
        {widget.config?.unit && <p className="text-sm text-slate-400 mt-1">{widget.config.unit}</p>}
      </div>
    );
  }
