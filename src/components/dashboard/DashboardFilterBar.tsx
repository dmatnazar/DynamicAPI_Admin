...
function MultiselectFilter({
  ...
  const allChecked = isAll || (opts.length > 0 && selected.length === opts.length);
    ...
    (async () => {
      ...
      try {
        ...
        for (const r of rows) {
          ...
          seen.add(v);
          next.push({ value: v, label: String(r[lf] ?? v) });
        }
        setOpts(next);
      } catch {
  ...
  function commit(ids: string[]) {
    // Hemmesi saýlandy → __ALL__ (API-a NULL / parametr iberilmeýär)
  ...
  function toggle(v: string) {
    const set = new Set(isAll ? opts.map((o) => o.value) : selected);
  ...
  const label =
    isAll || allChecked
      ? 'Hemmesi'
      : selected.length === 0
        ? filter.placeholder || 'Saýla…'
        : selected.length <= 2
          ? selected
              .map((v) => opts.find((o) => o.value === v)?.label || v)
              .join(', ')
          : `${selected.length} saýlandy`;
    ...
    <div className="relative min-w-[140px]">
      <button
        ...
        className={cn(
          ...
          selected.length > 0 && 'border-indigo-500/40 text-indigo-200'
        )}
      >
        {loading ? 'Ýüklenýär…' : label}
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setOpen(false)} />
          <div className="absolute z-50 top-full mt-1 left-0 right-0 max-h-56 overflow-y-auto rounded-xl border border-slate-700 bg-slate-900 shadow-xl p-2 space-y-0.5">
            {opts.length === 0 ? (
              <p className="text-[11px] text-slate-500 px-2 py-1.5">Maglumat ýok</p>
            ) : (
              <>
                <label className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-800 cursor-pointer text-xs text-slate-300 border-b border-slate-800 mb-0.5">
                  ...
                  />
                  <span className="font-medium">Hemmesini saýla</span>
                </label>
                {opts.map((o) => (
                  <label
                    key={o.value}
            ...
            {selected.length > 0 && (
              <button
                type="button"
                className="w-full text-[11px] text-slate-400 hover:text-rose-300 py-1"
                onClick={() => onChange('')}
              >
                Arassala
              </button>
              ...
              <div key={f.key} className="flex flex-wrap items-end gap-2">
                ...
                <div className="min-w-[140px]">
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="date"
                      value={String(values[f.endKey || 'endDate'] ?? '')}
