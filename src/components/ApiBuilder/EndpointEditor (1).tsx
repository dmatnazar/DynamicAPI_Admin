...
export function EndpointEditor({ endpoint, onChange, onOpenQueryEditor }: Props) {
  return (
    <div className="space-y-4 min-w-0">
      ...
      <div className="min-w-0">
        <div className="flex items-center justify-between mb-1.5">
          <h4 className="text-sm font-medium text-neutral-100">MSSQL Query</h4>
          <Button variant="secondary" className="!px-3 !py-1.5 !text-xs" onClick={onOpenQueryEditor}>
        ...
        <button
          ...
          className="w-full text-left rounded-lg border border-surface-border bg-surface-raised px-3 py-2.5 hover:border-accent/50 transition"
          title="Açmak üçin bas"
        >
          <pre className="text-xs font-mono text-neutral-400 whitespace-pre-wrap line-clamp-3 overflow-hidden">
            {endpoint.sqlQuery?.trim() || '-- Entek ýazylmady, açmak üçin bas'}
          </pre>
        </button>
      </div>
